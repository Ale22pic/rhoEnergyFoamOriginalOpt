...
//    Reference
//    D. Modesti and S. Pirozzoli A high-fidelity solver for turbulent compressible 
//    flows on unstructured meshes. Comput. & Fluids (2017)
//---------------------------------------------------------------------------
...
//      Check on sensor flags

        if (pressArtDiff || convArtDiff)
	{

	if (cycle == gamCoeff.size()-1)
	{
	...
	#include "sensor.H" 
        ...
        #include "AUSM.H"    // AUSM+up dissipation on pressure term
...
//
        ...
        //surfaceScalarField mylap = fvc::laplacian(U) ;
...
//      Total fluxes, Eulerian + viscous 
	...
	rhoFlux   = rhoave*phi ;
        ...
        #include "AUSM_conv.H"    // AUSM+up dissipation on convective terms
...
//      Till now, these are all quantities divided by the volume, 
//      since the divergence is applied in the next lines
        
        rhoFl = fvc::div(rhoFlux);
        momFl = fvc::div(momFlux) - fvc::div(tauMC);
        enFl  = fvc::div(enFlux)  ;

//      #include "pressureGrad.H" // forcing term to keep a 
